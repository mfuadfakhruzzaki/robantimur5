...
import { ArrowLeft, Trophy, Star, Target, Flame, Award, Users, CheckCircle, Loader2 } from "lucide-react"
    ...
    try {
      ...
      if (allBadges) {
        const badgesWithStatus = allBadges.map((badge) => ({
          ...badge,
          earned: userBadgeData?.some((ub) => ub.badge_id === badge.id) || false,
          earned_at: userBadgeData?.find((ub) => ub.badge_id === badge.id)?.earned_at,
        }))
        setUserBadges(badgesWithStatus)
      }

      // Fetch daily challenges with completion status
      const { data: allChallenges } = await supabase.from("daily_challenges").select("*").eq("is_active", true)
      ...
      if (allChallenges) {
        const challengesWithStatus = allChallenges.map((challenge) => ({
          ...challenge,
          completed: completedChallenges?.some((cc) => cc.challenge_id === challenge.id) || false,
        }))
      ...
      const { data: leaderboardData } = await supabase
        ...
        .select(`
          user_id,
          total_points,
          profiles!user_stats_user_id_fkey(name, avatar_url)
        `)
      ...
      <header className="bg-gradient-to-r from-purple-500 to-pink-500 text-white">
        <div className="container mx-auto px-4 py-6">
          ...
          <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{userStats.total_points.toLocaleString()}</div>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-gray-600">
                    {nextLevelXP - userStats.current_xp} XP lagi untuk naik ke Level {userStats.level + 1}
                  </p>
                </div>
              </CardContent>
            </Card>
                    ...
                    <div key={activity.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2 mt-1">
                          <UIBadge variant="secondary">+{activity.points_earned} poin</UIBadge>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-blue-600">{userBadges.filter((b) => b.earned).length}</div>
                  <div className="text-sm text-gray-600">Badge Diraih</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-green-600">{userStats.articles_read}</div>
                  <div className="text-sm text-gray-600">Artikel Dibaca</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-purple-600">{userStats.questions_asked}</div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {challenges.map((challenge) => (
                    ...
                    >
                      <div className={`text-2xl ${challenge.completed ? "grayscale-0" : "grayscale"}`}>
                        {challenge.icon}
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          <UIBadge variant={challenge.completed ? "default" : "secondary"}>
                      ...
                      {!challenge.completed && (
                        <Button size="sm" onClick={() => completeChallenge(challenge.id, challenge.points)}>
                          Mulai
                        </Button>
                      )}
                    </div>
          ...
          <TabsContent value="badges" className="space-y-6">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {userBadges.map((badge) => (
                <Card
                  key={badge.id}
                  className={`${badge.earned ? "border-yellow-200 bg-yellow-50" : "border-gray-200 bg-gray-50"}`}
                >
                  <CardContent className="p-4 text-center">
                    <div className={`text-4xl mb-3 ${badge.earned ? "" : "grayscale opacity-50"}`}>{badge.icon}</div>
                    <h4 className={`font-semibold mb-2 ${badge.earned ? "text-yellow-800" : "text-gray-600"}`}>
                      {badge.name}
                    </h4>
                    ...
                    {badge.earned ? (
                      <div className="space-y-2">
                        ...
                        {badge.earned_at && (
                          <p className="text-xs text-gray-500">
                            Diraih pada {new Date(badge.earned_at).toLocaleDateString("id-ID")}
                          </p>
                        )}
                      </div>
                    ) : (
                      <div className="space-y-2">
                        ...
                        <p className="text-xs text-gray-500">Butuh {badge.points_required} poin</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
                ...
                <div className="space-y-2">
                  {leaderboard.map((user) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <span className={`font-medium ${user.isCurrentUser ? "text-blue-700" : "text-gray-800"}`}>
                          {user.name} {user.isCurrentUser && "(Anda)"}
                        </span>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-gray-800">{user.total_points.toLocaleString()}</div>
